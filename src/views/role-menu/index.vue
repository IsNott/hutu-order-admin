<template>
  <div class="contianer">
    ...
    <div class="list">
      <el-table :data="roleMenuList" stripe style="width: 100%" border v-loading="loading">
        <template #empty>
          <img src="@/assets/img/empty.png" />
          <p style="font-size: 20px">Nothing to load....</p>
        </template>
        <el-table-column type="index" width="50" align="center" />
        <el-table-column prop="roleId" label="角色ID" width="120" align="center"></el-table-column>
        <el-table-column prop="menuId" label="菜单ID" width="120" align="center"></el-table-column>
...
const query = () => {
  loading.value = true
  roleMenuApi.queryPage(searchParam.value, page.value.page, page.value.size).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.message)
      return
    }
    roleMenuList.value = res.data.records
...
const handleSwitch = (row) => {
  roleMenuApi.update(row).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.message)
      return
    }
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除吗？', '提示', {
    ...
    roleMenuApi.delete(row.id).then(res => {
      if (res.code !== 200) {
        ElMessage.error(res.message)
        return
      }
