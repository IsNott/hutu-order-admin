<template>
    <div class="contianer">
        ...
        <div class="list">
            <el-table :data="userRoleList" stripe style="width: 100%" border v-loading="loading">
                <template #empty>
                    <img src="@/assets/img/empty.png" />
                    <p style="font-size: 20px">Nothing to load....</p>
                </template>
                <el-table-column type="index" width="50" align="center" />
                        <el-table-column prop="userId" label="系统用户id" width="120" align="center"></el-table-column>
...
<script setup>
    ...
    const query = () => {
        loading.value = true
        userRoleApi.queryPage(searchParam.value, page.value.page, page.value.size).then(res => {
            if (res.code !== 200) {
    ...
    const handleSwitch = (row) => {
        userRoleApi.update(row).then(res => {
            if (res.code !== 200) {
                ElMessage.error(res.message)
                return
            }
    ...
    const handleDelete = (row) => {
        ElMessageBox.confirm('确定要删除吗？', '提示', {
            ...
            userRoleApi.delete(row.id).then(res => {
                if (res.code !== 200) {
                    ElMessage.error(res.message)
                    return
                }
